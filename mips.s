mapreduce:
	...
	sw $s0 4($sp) # save argument 0 to the stack
	sw $s1, 8($sp)
	sw $s2, 12($sp)
	...
	lw $t1, 0($s0) # get the type (leaf/non-leaf)
	bne $t1, $zero, LEAF # branch to leaf when not equal

	lw $t2, 4($s0) # copy left child
	move $s0, $t2  # move left child to $a0

	jal mapreduce # recurse


	sw $v0, 16($sp) # save result to the stack
	lw $a0, 4($sp) # save a0 from the stack
	lw $s1, 8($sp) # save a1 to call mapreduce
	lw $s2, 12($sp) #save a2 to call mapreduce

	lw $a0, 8($a0) # save right addr
	#move $a0, $t3 # move right child to a0


 	lw $t2, 0($s0) # get the type (leaf/non-leaf)
	bne $t2, $zero, LEAF # branch if type is leaf
	jal mapreduce # traverse left child

	lw $s0, 16($sp) # pop first v0 to the stack
	move $s1, $v0 # get the return value 
	jalr $s2 # jump to the reduce function
...
LEAF:
	...
	lw  $s0, 4($s0) # load address of str pointer
	jalr $s1 # jump to mapper function 
