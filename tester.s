mapreduce:
	addi $sp, $sp -20 # allocate for 4 words 
	sw $ra 0($sp) # save return register to the stack
	sw $a0 4($sp) # save argument 0 to the stack
	sw $a1, 8($sp)
	sw $a2, 12($sp)

	lw $t1, 0($a0) # get the type (leaf/non-leaf)
	bne $t1, $zero, LEAF # branch to leaf when not equal

	lw $t2, 4($a0) # copy left child
	move $a0, $t2  # move left child to $a0

	jal mapreduce # recurse on left child

	lw $a0, 4($sp) # save a0 from the stack
	lw $a1, 8($sp) # save a1 to call mapreduce
	lw $a2, 12($sp) #save a2 to call mapreduce
	sw $v0, 16($sp) # save result to the stack

	lw $t2, 0($a0) # get the type (leaf/non-leaf)
	bne $t2, $zero, LEAF # branch if type is leaf

	lw $a0, 8($a0) # save right addr
	#move $a0, $t3 # move right child to a0
 	
	jal mapreduce # traverse right child

	lw $a0, 16($sp) # pop first v0 to the stack
	move $a1, $v0 # get the return value 
	jalr $a2 # jump to the reduce function
	lw $ra ,0($sp) # return address
	addiu $sp, $sp, 20 # fill the stack
	jr $ra

LEAF:

	lw  $a0, 4($a0) # load address of str pointer
	jalr $a1 # jump to mapper function 
	move $t1, $v0 # move mapper value to temp
	lw $ra 0($sp) # restore the return value
	addiu $sp, $sp, 20 # deallocating 
